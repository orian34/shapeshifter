...
{
    public class PrimordialFire : ModProjectile
    {
        public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Primordial Fire");
		}
		...
		public override void AI()
		{
			...
			{
				int newDust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 57, 0f, -1f, 0, default(Color));
		   ...
		   projectile.position.X = player.position.X+128;
		   ...
		   for(int i = 0; i < 1001; i++)
			{
			   ...
			   if(target.type == mod.ProjectileType("PrimordialMissile") && target.owner == projectile.owner)
			   {
				   if(projectile.getRect().Intersects(target.getRect()))
				   {
					   if(!sp.primeFire)
					   {
						   string str = "You get infused with the knowledge of primordial energy. You can now make it flow freely out of an enemy!";
							Main.NewText( str, 0, 150, 150, false );
					   }
					   sp.primeFire = true;
					   sp.primeTimer = 600f;
